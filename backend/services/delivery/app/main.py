...
from app.consumers.restaurant_events import start_restaurant_consumer
...
async def lifespan(app: FastAPI):
    logger.info("Delivery service starting up", extra={
...
app = FastAPI(
    ...
    servers=[{"url": "http://localhost:8080/api/deliveries", "description": "API Gateway"}]
)
...
async def readiness():
    ...
    if not db_ready:
        return JSONResponse(
            status_code=503,
            content={"status": "not ready", "service": "delivery", "database": "not connected"}
        )
...
@app.get("/health/live")
async def liveness():
    return {"status": "alive", "service": "delivery"}
