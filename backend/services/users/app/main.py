...
async def lifespan(app: FastAPI):
    logger.info("Users service starting up", extra={
...
app = FastAPI(
    ...
    servers=[{"url": "http://localhost:8080/api/users", "description": "API Gateway"}]
)
...
async def readiness():
    ...
    if not db_ready:
        return JSONResponse(
            status_code=503,
            content={"status": "not ready", "service": "users", "database": "not connected"}
        )
