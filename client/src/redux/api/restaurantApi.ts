...
export const restaurantApi = api.injectEndpoints({
  endpoints: (builder) => ({
    addRestaurant: builder.mutation<GenericResponse, AddRestaurantRequest | Record<string, unknown> | FormData>({
      query: (data) => ({
        url: '/restaurant/',
        method: 'POST',
        body: data,
      }),
      invalidatesTags: ['my-restaurants', 'restaurant-dashboard'],
    }),
    ...
    updateRestaurant: builder.mutation<GenericResponse, UpdateRestaurantRequest>({
      ...
      invalidatesTags: ['my-restaurants', 'a-restaurant'],
    }),
    ...
    getRestaurantById: builder.query<GetRestaurantDetailsResponse, string>({
      ...
      providesTags: (_result, _error, id) => [{ type: 'a-restaurant', id }],
    }),
