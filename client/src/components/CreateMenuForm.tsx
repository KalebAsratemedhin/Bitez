...
type CreateMenuFormData = {
  title: string;
  menuItems: { name: string; description: string; price: number; itemPicture: unknown }[];
};
...
const CreateMenuForm = ({ selectedRestaurantId }: Props) => {
  const {
    ...
    resolver: zodResolver(createMenuSchema) as Resolver<CreateMenuFormData>,
    defaultValues: {
  ...
  const onSubmit = async (data: CreateMenuFormData) => {
    try {
      ...
      data.menuItems.forEach((item) => {
        if (item.itemPicture && item.itemPicture instanceof File) formData.append("itemPictures", item.itemPicture);
      });
