...
import { upsertOrderReadModel, upsertRestaurantReadModel, upsertDeliveryReadModel, } from "../repositories/ReadModelRepositories.js";
...
export async function startDashboardEventConsumers(url) {
    const connection = await amqp.connect(url);
    const channel = await connection.createChannel();
    await channel.assertExchange(EXCHANGE, EXCHANGE_TYPE, { durable: true });
    const bindAndConsume = async (queue, keys, handler) => {
        ...
        await channel.consume(queue, async (msg) => {
            ...
            try {
                ...
                console.error(`Dashboard consumer ${queue} error:`, err);
                channel.nack(msg, false, true);
            }
        });
    };
    await bindAndConsume(QUEUES.order, [ROUTING_KEYS.orderCreated, ROUTING_KEYS.orderUpdated], async (p) => {
    ...
    await bindAndConsume(QUEUES.restaurant, [ROUTING_KEYS.restaurantCreated, ROUTING_KEYS.restaurantUpdated], async (p) => {
        const restaurantId = String(p.restaurantId ?? "");
    ...
    await bindAndConsume(QUEUES.delivery, [ROUTING_KEYS.deliveryCreated, ROUTING_KEYS.deliveryUpdated], async (p) => {
    ...
    console.log("Dashboard: consuming order.*, restaurant.*, delivery.*");
}
