import type { IMenuRepository, IRestaurantRepository } from "@domain/interfaces/index.js";
import type {
  CreateMenuInput,
  UpdateMenuInput,
...
export class MenuUseCase {
  ...
  async update(input: UpdateMenuInput): Promise<unknown> {
    ...
    const restaurantId = m.restaurant && typeof m.restaurant === "object" && "_id" in m.restaurant
      ...
      : String(m.restaurant);
    await this.assertRestaurantOwner(restaurantId, userId);
    const update: { menuName?: string; menuItems?: { name: string; description: string; price: number; itemPicture?: string }[] } = {};
